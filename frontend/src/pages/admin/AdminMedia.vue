    ...
    <div class="table-card-modern">
      ...
      <div v-for="item in mediaList" :key="item.id_media" class="table-row-modern media-grid has-divider">
        ...
        <span class="actions-modern">
          ...
          <button type="button" class="icon-btn delete" @click.prevent="openDeleteModal(item.id_media)" title="Hapus">
    ...
    <div v-if="isDeleteModalOpen" class="modal-overlay">
      <div class="form-modal fade-in-up" style="max-width: 400px; text-align: center;">
        ...
        <p class="modal-text" style="margin: 0 0 18px 0; color: #4b5563; font-size: 14px; line-height: 1.5;">
          Apakah Anda yakin ingin menghapus data media tanam ini?
        </p>
...
async function saveMedia() {
  try {
    ...
    showNotification('Data media berhasil disimpan!', 'success')
    closeForm()
  } catch (error) {
    console.error('Error simpan media:', error)
    showNotification('Gagal menyimpan data media tanam.', 'error')
  }
}
...
async function executeDelete() {
  ...
  try {
    await mediaTanamService.delete(idToDelete.value)
    showNotification('Media tanam berhasil dihapus.', 'success')
    await loadMedia()
    eventBus.emit('refreshBudidayaData')
  } catch (error) {
    console.error('Error hapus media:', error)
    showNotification('Gagal menghapus data media tanam.', 'error')
  } finally {
    closeDeleteModal()
  }
}
